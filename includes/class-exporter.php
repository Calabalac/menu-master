...
class MenuMaster_Exporter {
    
    public function __construct() {
        add_action('init', array($this, 'handle_export_requests'));
        add_action('wp_ajax_menu_master_export', array($this, 'ajax_export'));
    }
    ...
    public function handle_export_requests() {
        if (!isset($_GET['menu_master_export'])) {
            return;
        }
        
        $action = sanitize_text_field($_GET['menu_master_export']);
        $catalog_id = intval($_GET['catalog_id']);
        $format = sanitize_text_field($_GET['format']);
        
        MenuMaster_Logger::info('🔄 Export request received', array(
            'action' => $action,
        ...
        if (!current_user_can('manage_options') && $action !== 'feed') {
            MenuMaster_Logger::error('❌ Export access denied', array(
                'action' => $action,
        ...
        if (!$catalog_id) {
            MenuMaster_Logger::error('❌ Export invalid catalog ID', array(
                'catalog_id' => $catalog_id
            ));
        ...
        switch ($action) {
            case 'download':
                MenuMaster_Logger::info('📥 Processing download export', array(
                    'catalog_id' => $catalog_id,
                    'format' => $format
                ));
                ...
                break;
            case 'feed':
                MenuMaster_Logger::info('📡 Processing feed export', array(
                    'catalog_id' => $catalog_id,
                    'format' => $format
                ));
                ...
                break;
            default:
                MenuMaster_Logger::error('❌ Invalid export action', array(
                    'action' => $action,
                    'catalog_id' => $catalog_id,
                    'format' => $format
                ));
    ...
    private function export_download($catalog_id, $format) {
        ...
        $items = MenuMaster_Database::get_catalog_items($catalog_id);
    ...
    private function export_feed($catalog_id, $format) {
        ...
        switch ($format) {
            case 'csv':
                MenuMaster_Logger::info('📄 Exporting CSV feed', array('items_count' => count($items)));
                $this->export_csv($catalog, $items, false);
                break;
            case 'excel':
                MenuMaster_Logger::info('📊 Exporting Excel feed', array('items_count' => count($items)));
                $this->export_excel($catalog, $items, false);
                break;
            case 'json':
                MenuMaster_Logger::info('🔗 Exporting JSON feed', array('items_count' => count($items)));
                $this->export_json($catalog, $items);
                break;
            case 'xml':
                MenuMaster_Logger::info('📝 Exporting XML feed', array('items_count' => count($items)));
                $this->export_xml($catalog, $items);
                break;
            default:
                MenuMaster_Logger::error('❌ Invalid export format in feed', array(
                    'format' => $format,
                    'catalog_id' => $catalog_id,
                    'supported_formats' => ['csv', 'excel', 'json', 'xml']
                ));
                wp_die('Invalid format: ' . $format . '. Supported formats: csv, excel, json, xml');
        }
    }
    ...
    private function create_xlsx_file($catalog, $items) {
        ...
        $temp_dir = sys_get_temp_dir() . '/menu_master_' . uniqid();
        mkdir($temp_dir);
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
    <sheetData>';
        ...
        // Create ZIP archive
        $zip = new ZipArchive();
        $zip_file = tempnam(sys_get_temp_dir(), 'menu_master_xlsx');
