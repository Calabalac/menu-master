...
class MenuMaster_Logger {
    ...
    public static function init() {
        ...
        self::$log_file = $upload_dir['basedir'] . '/menu-master-debug.log';
        
        // Enable debug mode if WP_DEBUG is on or plugin debug option is set
        self::$debug_mode = defined('WP_DEBUG') && WP_DEBUG || get_option('menu_master_debug', false);
    }
    ...
    private static function log($level, $message, $context = array()) {
        ...
        if (defined('WP_DEBUG_LOG') && WP_DEBUG_LOG) {
            error_log("Menu Master [{$level}]: {$message}");
        }
    }
    ...
    public static function get_recent_logs($lines = 50) {
        ...
        foreach ($recent_lines as $line) {
            if (preg_match('/^\[([^\]]+)\]\s+(\w+)\s+\[User:\s*([^\]]*)\]\s+(.*)$/', $line, $matches)) {
                $parsed_logs[] = [
                    'timestamp' => $matches[1],
                    'level' => $matches[2],
                    'user' => $matches[3],
    ...
    public static function enable_debug() {
        update_option('menu_master_debug', true);
    ...
    public static function disable_debug() {
        update_option('menu_master_debug', false);
