...
class MenuMaster_Database {
    
    public static function create_tables() {
        global $wpdb;
        
        // Log the start of table creation
        if (class_exists('MenuMaster_Logger')) {
            MenuMaster_Logger::info('Starting database table creation');
        }
        ...
        // Table for catalogs
        $table_catalogs = $wpdb->prefix . 'menu_master_catalogs';
        ...
        $table_mapping = $wpdb->prefix . 'menu_master_columns_mapping';
        $sql_mapping = "CREATE TABLE $table_mapping (
          ...
          // Table for image reviews
        $table_reviews = $wpdb->prefix . 'menu_master_image_reviews';
        ...
        $table_items = $wpdb->prefix . 'menu_master_items';
        ...
        $tables_to_create = [
            ...
            'reviews' => [$table_reviews, $sql_reviews]
        ];
        ...
        foreach ($tables_to_create as $type => $table_info) {
            ...
            if (!$table_exists) {
                if (class_exists('MenuMaster_Logger')) {
                    MenuMaster_Logger::debug("Creating table: {$table_name}");
                }
                ...
                if ($table_exists_after) {
                    $tables_created++;
                    if (class_exists('MenuMaster_Logger')) {
                        MenuMaster_Logger::info("Successfully created table: {$table_name}");
                    }
                } else {
                    ...
                    $errors[] = $error_msg;
                    if (class_exists('MenuMaster_Logger')) {
                        MenuMaster_Logger::error($error_msg);
                    }
                }
            } else {
                if (class_exists('MenuMaster_Logger')) {
                    MenuMaster_Logger::info("Table already exists: {$table_name}");
                }
                $tables_created++;
            }
        }
        ...
        $menu_images_dir = $upload_dir['basedir'] . '/menu-master-images';
        if (!file_exists($menu_images_dir)) {
            if (wp_mkdir_p($menu_images_dir)) {
                if (class_exists('MenuMaster_Logger')) {
                    MenuMaster_Logger::info("Created images directory: {$menu_images_dir}");
                }
            } else {
                $error_msg = "Failed to create images directory: {$menu_images_dir}";
                $errors[] = $error_msg;
                if (class_exists('MenuMaster_Logger')) {
                    MenuMaster_Logger::error($error_msg);
                }
        ...
        $htaccess_file = $menu_images_dir . '/.htaccess';
        if (!file_exists($htaccess_file)) {
            $htaccess_content = "Options -Indexes\nDeny from all\n<Files ~ \"\\.(jpg|jpeg|png|gif|webp)$\">\n    Allow from all\n</Files>";
            if (file_put_contents($htaccess_file, $htaccess_content)) {
                if (class_exists('MenuMaster_Logger')) {
                    MenuMaster_Logger::info("Created .htaccess security file");
                }
            } else {
                $error_msg = "Failed to create .htaccess security file";
                $errors[] = $error_msg;
                if (class_exists('MenuMaster_Logger')) {
                    MenuMaster_Logger::warning($error_msg);
                }
            }
        }
        ...
        if (class_exists('MenuMaster_Logger')) {
            if (empty($errors)) {
                MenuMaster_Logger::info("Database setup completed successfully. Tables created/verified: {$tables_created}");
            } else {
                MenuMaster_Logger::error("Database setup completed with errors", ['errors' => $errors, 'tables_created' => $tables_created]);
            }
        }
    ...
    public static function drop_tables() {
        ...
        $tables = array(
            $wpdb->prefix . 'menu_master_items',
            $wpdb->prefix . 'menu_master_columns_mapping',
            $wpdb->prefix . 'menu_master_catalogs',
        ...
        $menu_images_dir = $upload_dir['basedir'] . '/menu-master-images';
        if (file_exists($menu_images_dir)) {
            self::remove_directory($menu_images_dir);
        }
    }
    ...
    public static function get_catalogs() {
        global $wpdb;
        $table = $wpdb->prefix . 'menu_master_catalogs';
    ...
    public static function create_catalog($data) {
        ...
        MenuMaster_Logger::debug('Creating catalog with data', $data);
        
        // Validate required fields
        if (empty($data['name'])) {
            MenuMaster_Logger::error('Cannot create catalog: name is required');
            return false;
        }
        ...
        if (!$table_exists) {
            MenuMaster_Logger::error('Table does not exist: ' . $table);
            return false;
        }
        ...
        MenuMaster_Logger::debug('Inserting data into table: ' . $table, $insert_data);
        
        $result = $wpdb->insert($table, $insert_data);
        
        if ($result === false) {
            MenuMaster_Logger::db_error(
                'create_catalog',
        ...
        if ($insert_id) {
            MenuMaster_Logger::info('Catalog created successfully with ID: ' . $insert_id);
        } else {
            MenuMaster_Logger::warning('Insert succeeded but no insert_id returned');
        }
    ...
    public static function delete_catalog($id) {
        ...
        // Delete mapping
        $mapping_table = $wpdb->prefix . 'menu_master_columns_mapping';
        $wpdb->delete($mapping_table, array('catalog_id' => $id));
        
        // Delete image reviews
        $reviews_table = $wpdb->prefix . 'menu_master_image_reviews';
        $wpdb->delete($reviews_table, array('catalog_id' => $id));
        
        // Delete catalog
        $catalog_table = $wpdb->prefix . 'menu_master_catalogs';
        return $wpdb->delete($catalog_table, array('id' => $id));
    }
    
    public static function get_catalog_items($catalog_id, $limit = 0, $offset = 0) {
        global $wpdb;
        $table = $wpdb->prefix . 'menu_master_items';
    ...
    public static function get_column_mapping($catalog_id) {
        global $wpdb;
        $table = $wpdb->prefix . 'menu_master_columns_mapping';
