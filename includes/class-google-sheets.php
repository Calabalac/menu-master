...
class MenuMaster_GoogleSheets {
     ...
     */
    private static function convert_to_csv_url($url, $sheet_name = 'Sheet1') {
        // Extract spreadsheet ID from various URL formats
        if (!preg_match('~/d/([a-zA-Z0-9-_]+)~', $url, $m)) {
            MenuMaster_Logger::error('Invalid Google Sheets URL format', ['url' => $url]);
            return false;
        }
        ...
        // For now, we'll use GID 0 for all sheets since we can't easily map sheet names to GIDs
        // In a real implementation, you'd need to use the Google Sheets API to get the GID for a sheet name
        
        $csv_url = "https://docs.google.com/spreadsheets/d/{$spreadsheet_id}/export?format=csv&gid={$gid}";
        
        MenuMaster_Logger::info('Generated CSV URL', [
            'original_url' => $url,
            'csv_url' => $csv_url,
